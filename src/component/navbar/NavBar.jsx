...
export default function NavBar() {
  const [elementHeight, setElementHeight] = useState("py-4");
  window.addEventListener('scroll',function handell () {
      let scrolling = window.scrollY;
      if (scrolling >= 100) {
        setElementHeight("")
        
      } else {
        setElementHeight("py-4")
       }
    } )
  return (
    <>
      <nav className={`navbar navbar-expand-lg  ${elementHeight}`}  >
